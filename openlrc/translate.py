...
class LLMTranslator(Translator):
    ...
    def make_chunks_by_tokens(self, texts: list[str]) -> list[list[tuple[int, str]]]:
        ...
        # or the merged result would exceed the token budget.
        if len(chunks) >= 2:
            tail = chunks[-1]
            tail_tokens = sum(token_counts[c[0] - 1] for c in tail)
            prev_tokens = sum(token_counts[c[0] - 1] for c in chunks[-2])
            is_small_tail = len(tail) < self.chunk_size / 2 and tail_tokens < self.MAX_CHUNK_TOKENS / 2
            would_exceed = (prev_tokens + tail_tokens) > self.MAX_CHUNK_TOKENS or (
                len(chunks[-2]) + len(tail)
            ) > self.chunk_size
            ...
            if is_small_tail and not scene_break and not would_exceed:
