...
class TestMakeChunksByTokens(unittest.TestCase):
    ...
    def test_small_tail_merged(self):
        """A small trailing chunk is merged into the previous one when it fits."""
        translator = self._make_translator(chunk_size=10, max_chunk_tokens=10000)
        texts = [f"text{i}" for i in range(7)]  # chunk_size=10, so 7 lines fit in one chunk
        chunks = translator.make_chunks_by_tokens(texts)
        self.assertEqual(len(chunks), 1)

    def test_small_tail_not_merged_if_would_exceed(self):
        """A small trailing chunk is NOT merged if the result would exceed token budget or line cap."""
        translator = self._make_translator(chunk_size=5, max_chunk_tokens=10000)
        texts = [f"text{i}" for i in range(7)]  # 5 + 2, merge would give 7 > chunk_size(5)
        chunks = translator.make_chunks_by_tokens(texts)
        self.assertEqual(len(chunks), 2)
